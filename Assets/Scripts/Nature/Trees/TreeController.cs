...
namespace Tree
{
    ...
    */
    ...
    {
        [SerializeField] private GameObject _stumpGameObj;
        [SerializeField] private GameObject _trunkGameObj;
        ...
        private void Awake()
        {
            if (_stumpGameObj == null || _trunkGameObj == null)
            {
                Debug.LogError("Error: Tree parent gameObj must have trunk and stump defined in the editor");
            }
        }


        public void TakeDamageFromChild(float damageAmount)
        {
            //Verify the child components don't call this once the tree has been damaged to the point of falling over. 
            if (_health <= 0)
            {
                Debug.LogError("Error: Cannot call 'TakeDamageFromChild' on tree that has fallen over!");
                return;
            }

            _health -= damageAmount;

            //If health of standing tree is zero (at which point it will fall), than we tell the stump and trunk to take individual damage, rather that overall damage.
            if (_health <= 0f)
            {
                _stumpGameObj.GetComponent<TreeStumpController>().OnTreeTrunkFall();
                _trunkGameObj.GetComponent<TreeTrunkController>().FallOver();
            }
        }
